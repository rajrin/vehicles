...
func (t *SimpleChaincode) Init(stub shim.ChaincodeStubInterface, function string, args []string) ([]byte, error) {
	...
	if err != nil {
		return nil, errors.New("Error creating V5C_Holder record")
	}
	...
	if function == "create_vehicle" {
		...
		if strings.Contains(function, "update") == false &&
			...
			if function == "authority_to_manufacturer" {
				return t.authority_to_manufacturer(stub, v, caller, caller_affiliation, args[0], rec_affiliation)
			} else if function == "manufacturer_to_private" {
				return t.manufacturer_to_private(stub, v, caller, caller_affiliation, args[0], rec_affiliation)
			} else if function == "private_to_private" {
				return t.private_to_private(stub, v, caller, caller_affiliation, args[0], rec_affiliation)
